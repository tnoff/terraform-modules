...
locals {
  # kubelet flags appended to the default OKE init below. Built as a list so the
  # optional --register-with-taints reads as a sibling flag and there is no
  # trailing space when node_taints is empty (keeps taint-free pools' cloud-init
  # byte-identical). Taints apply at node registration (effect uses the k8s
  # spelling, e.g. NoSchedule), only when enable_node_init_customizations is
  # true, and only on newly-registered nodes (recycle existing nodes to adopt).
  kubelet_extra_args = join(" ", concat(
    [
      "--image-gc-high-threshold=${var.image_gc_high_threshold_percent}",
      "--image-gc-low-threshold=${var.image_gc_low_threshold_percent}",
    ],
    length(var.node_taints) > 0 ? ["--register-with-taints=${join(",", [for t in var.node_taints : "${t.key}=${t.value}:${t.effect}"])}"] : [],
  ))
