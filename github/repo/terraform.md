...
| <a name="input_allow_update_branch"></a> [allow\_update\_branch](#input\_allow\_update\_branch) | Always suggest updating pull request branches. Under a ruleset with strict\_required\_status\_checks\_policy, a PR whose base has moved is BEHIND and cannot merge; GitHub's auto-merge does not update it on its own, so Renovate PRs sit green-and-approved until Renovate's next run rebases them. Defaults to false, matching the provider, so turning it on is a per-repo decision. | `bool` | `false` | no |
