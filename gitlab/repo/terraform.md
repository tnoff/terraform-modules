...
|------|-------------|------|---------|:--------:|
| <a name="input_auto_cancel_pending_pipelines"></a> [auto\_cancel\_pending\_pipelines](#input\_auto\_cancel\_pending\_pipelines) | Auto-cancel older pending pipelines on a ref when a newer pipeline starts. "enabled" or "disabled". | `string` | `"enabled"` | no |
...
| <a name="input_push_access_level"></a> [push\_access\_level](#input\_push\_access\_level) | Who can push directly to the default branch. Defaults to 'no one' so all changes go through MRs; raise to 'maintainer' to allow emergency direct pushes. | `string` | `"no one"` | no |
